...
import { GetClubMemberLevel, GetConfiguration, GetSessionDataManager, LocalizeText } from '../../api';
import { Base, Flex, Grid, LayoutCurrencyIcon, NitroCardContentView, NitroCardHeaderView, NitroCardTabsItemView, NitroCardTabsView, NitroCardView, Text } from '../../common';
...
export const BackgroundsView: FC<BackgroundsViewProps> = ({
    ...
    return (
        <NitroCardView uniqueKey="backgrounds" className="nitro-backgrounds no-resize" theme="primary">
            <NitroCardHeaderView headerText={LocalizeText('infostand.background.header')} onCloseClick={() => setIsVisible(false)} />
            <NitroCardTabsView>
                {TABS.map(tab => (
            ...
            <NitroCardContentView gap={1}>
                <Text bold center>{LocalizeText('infostand.background.text')} </Text>
